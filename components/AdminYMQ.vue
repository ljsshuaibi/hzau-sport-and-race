<template>
    ...
    <div class="main">
      ...
      <div class="info">
        <el-descriptions  border="true" column="2">
          <el-descriptions-item label="运动场名称">羽毛球</el-descriptions-item>
  ...
  export default {
   ...
   data() {
    ...
    methods: {
        getData() {
      axios.post('http://127.0.0.1:80/api/userInfo/ymq').then(response => {
    ...
    disable(index) {
      ...
      axios.post('http://127.0.0.1:80/api/adminInfo/disenableColumn/ymq',Info,{
        headers: {
    ...
    enable(index) {
      ...
      axios.post('http://127.0.0.1:80/api/adminInfo/enableColumn/ymq',Info,{
        headers: {
          'Content-Type': 'application/json'
        }}).then(response=>{
          if(response.data===1){
